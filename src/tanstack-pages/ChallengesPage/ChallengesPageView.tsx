...
const Challenges: React.FC = () => {
  ...
  return (
    <div className=" flex flex-col items-start ">
      <div className="fixed z-10 flex justify-between w-full bg-black h-[8vh] items-center pl-3 top-0">
        <button
          onClick={() => {
            if (!isOnBoarding) {
              navigate({ to: "/welcome" });
      ...
      {categories.map((category, categoryIndex) => (
        <div key={categoryIndex} className="mb-8 pl-3">
          ...
          <div className={`flex  overflow-x-scroll  gap-4 pb-4 w-screen `}>
            {category.items.map((card, cardIndex) => (
              <Link
                ...
                style={{ transform: "t" }}>
                <div className="z-10 text-outline font-druk absolute inset-x-0 top-0 p-3 text-start text-lg leading-7 text-black">
                  {card.title}
                </div>
                <Image
                  src={card.imageUrl}
